...
from utils.ddd_utils import comput_corners_3d, alpha2rot_y
...
class GenericDataset(data.Dataset):
    ...
    def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
        ...
        if opt is not None and split is not None:
            ...
            self.enable_meta = True if (opt.run_dataset_eval and split in ["val", "mini_val",
                                                                           "test"]) or opt.eval else False
        ...
        if opt.tracking:
            ...
            if opt.pointcloud:
                pre_pc_2d, pre_pc_N, pre_pc_hm, pre_pc_3d = self._load_pc_data(pre_img, pre_img_info,
                                                                               trans_input_pre, trans_output_pre,
                                                                               flipped)
    ...
    def create_pc_pillars(self, img, img_info, pc_2d, pc_3d, inp_trans, out_trans):
        ...
        if self.opt.debug:
            ...
            for i, p in enumerate(pc_inp[:3, :].T):
                ...
                rect_tl_ori = (
                np.min(int(pc_2d[0, i] - pill_wh_ori[0, i] / 2), 0), np.min(int(pc_2d[1, i] - pill_wh_ori[1, i]), 0))
