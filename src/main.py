...
def main(opt):
    ...
    opt = opts().update_dataset_info_and_set_heads(opt, NuScenes)
    print(opt)
    ...
    if opt.val_intervals < opt.num_epochs or opt.eval:
        ...
        val_loader = torch.utils.data.DataLoader(
            NuScenes(opt, opt.val_split), batch_size=1, shuffle=False,
    ...
    train_loader = torch.utils.data.DataLoader(
        NuScenes(opt, opt.train_split), batch_size=opt.batch_size,
