...
class RadarPointCloudWithVelocity(RadarPointCloud):
    ...
    def from_file_multisweep(cls,
        ...
        ref_from_car_rot = transform_matrix(np.array([0.0, 0.0, 0.0]), Quaternion(ref_cs_rec['rotation']), inverse=True)

        # Homogeneous transformation matrix from global to _current_ ego car frame.
        car_from_global = transform_matrix(ref_pose_rec['translation'], Quaternion(ref_pose_rec['rotation']),
                                           inverse=True)
        car_from_global_rot = transform_matrix(np.array([0.0, 0.0, 0.0]),
        ...
        for _ in range(nsweeps):
            ...
            global_from_car_rot = transform_matrix(np.array([0.0, 0.0, 0.0]),
                                                   Quaternion(current_pose_rec['rotation']), inverse=False)
            ...
            car_from_current = transform_matrix(current_cs_rec['translation'], Quaternion(current_cs_rec['rotation']),
                                                inverse=False)
            car_from_current_rot = transform_matrix(np.array([0.0, 0.0, 0.0]), Quaternion(current_cs_rec['rotation']),
                                                    inverse=False)
...
def get_alpha(rot):
    ...
    alpha = alpha1 * float(idx) + alpha2 * float(~idx)
    return alpha
